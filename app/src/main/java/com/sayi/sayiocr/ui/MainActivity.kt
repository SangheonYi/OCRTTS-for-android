...
import com.example.sayiocr.databinding.ActivityMainBinding
import com.google.android.gms.ads.AdRequest
import com.google.android.material.dialog.MaterialAlertDialogBuilder
import com.googlecode.tesseract.android.TessBaseAPI
import com.leinardi.android.speeddial.SpeedDialView
import com.sayi.sayiocr.*
import com.sayi.sayiocr.ui.transservice.TransService
import com.sayi.sayiocr.util.PermissionUtil
import java.io.*
import java.util.*
...
class MainActivity : AppCompatActivity(), OnInitListener, View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bindings.adView.loadAd(AdRequest.Builder().build())
        ...
        // 터치 이벤트 제거
        bindings.editOcrResult.setOnTouchListener { view, event -> true }
        bindings.speedDial.setOnActionSelectedListener(selectedListener)
    ...
    override fun onClick(src: View) {
        when (src.id) {
            ...
            R.id.album -> if (model.ocrIndex < 0) albumClick()
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == RESULT_OK) {
            Log.i("Intent size", "${data!!.clipData!!.itemCount}")
...
//                            startActivityForResult(intent, model.PICTURE_REQUEST_CODE)
    ...
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            else bindings.play.isEnabled = true
        }
    }
    ...
    public override fun onDestroy() {
        ...
        if (model.threadIndex > 0 && model.safUri != null) {
            model.ocrResult = bindings.editOcrResult.text.toString()
