    ...
    protected synchronized void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            if (requestCode == PICTURE_REQUEST_CODE) {
                ...
                if (pickedNumber > 0) {
                    OCR thread = new OCR(model.threadIndex, clipData);// OCR 진행할 스레드
...
//                    startService(new Intent(this, TransService.class));
                ...
                if (data != null) {
                    ...
                    Cursor cursor = getContentResolver().query(model.SAFUri, null, null, null, null);
    ...
    private class OCR extends Thread {
        ...
        public synchronized void run() {
            ...
            Uri urione;
            if (model.Page < clipData.getItemCount())
                model.OCRIndex = 0;
            Log.i("OCR", this.threadNum + "번째 스레드의 run");

            while (model.Page < clipData.getItemCount()) {
                try {
                    urione = clipData.getItemAt(model.Page).getUri();
            ...
            Log.i("OCR", "스레드 끝남");
//            stopService(new Intent(getApplicationContext(), TransService.class));
        }
    }
